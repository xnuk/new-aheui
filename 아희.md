...
1. 입력값에서 `-`(U+002D) 문자가 구간 중 가장 최근 위치에 있거나 없고 `0`(U+0030) 이상 `9`(U+0039) 이하의 문자들이 1개 이상 연속되어 나열되어 있는 구간들 중 가장 최근에 입력된 구간을 찾습니다.
2. 해당 구간을 부호 있는 10진수 숫자를 나타낸 문자열로 취급하여 해당 구간을 숫자로 변환한 값을 저장공간에 넣습니다. 최근에 입력된 숫자 문자일수록 높은 자리를 뜻하는 수가 됩니다.
